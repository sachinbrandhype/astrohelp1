...
const { booking_status_function, end_live_session_function, astrologer_dynamic_function, set_astrologer_comission_to_order } = require("../controllers/home.controller");
const {
  User,
  Bookingrequest,
  Broadcastjoin,
...
const { send_gifts_function } = require("../controllers/gift.controller");
...
io.on("connection", (socket) => {
  ...
  // const crnt = currentTimeStamp();
  // console.log("CURRENT", crnt);

  // io.use(cors());
  // io.use(bodyParser.json());
  // io.use(bodyParser.urlencoded({extended:true}));
  ...
  socket.on('end_live_session_by_astrologer',async (req)=>{
    ...
    if(end_session.status){
      ...
      // io.sockets.in(user_id).emit('end_live_session_by_astrologer',end_session);
  ...
  const complete_booking_broadcast = async (booking_id,user_id=0) => {
    // socket.join(bridge_id);
    ...
    if(bks){
      ...
      if(update){
        ...
        next_booking(bks.bridge_id,user_id)
        return true;
      }else{
        return false;
      }
  ...
  socket.on('complete_booking_broadcast',async (req)=>{
    ...
    socket.join(bridge_id);
    await complete_booking_broadcast(booking_id,user_id);
    io
    .sockets
    .in(bridge_id)
    ...
    await broadcast_joiners(bridge_id)
  ...
  socket.on('cancel_broadcast_join',async(req)=>{
    ...
    if(bks){
      ...
      try {
        ...
        next_booking(bridge_id,bks.user_id)
  ...
  const broadcast_joiners = async (bridge_id) => {
    socket.join(bridge_id);
      ...
      if(!check_if_user_has_already_booking){
        ...
        try {
            ...
            if(!check_max_book_queue){
              ...
              complete_broadcast_booking_by_interval(bookingInsertID,user_id);
  ...
  const complete_broadcast_booking_by_interval = async (booking_id,user_id=0) => {
    ...
    broadcast_joiners_timeout= setTimeout(async () => {
      ...
      broadcast_time_expired_user(user_id,booking2.user_id)
      socket.join(booking2.bridge_id);

      /**next timeout */
      next_booking(booking2.bridge_id,user_id)
      ...
      if(booking2){
        ...
        broadcast_joiners(booking2.bridge_id)
  ...
  const next_booking = async (bridge_id,user_id=0) => {
    /**next timeout */
    socket.join(bridge_id);
    ...
    if(nextbooking){
      complete_broadcast_booking_by_interval(nextbooking.id,user_id)
    }
